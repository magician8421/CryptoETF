...
contract CryptoETFTokenOralce{
    ...
    function price(address etfAddress,address tokenIn, uint32 secondsAgo) external returns(uint256){
      ...
      for(uint256 i=0;i<_cons.length;i++){
        ...
        uint128 _tokenAmount=uint128(10)**IERC20Metadata(etfAddress).decimals()*_distribution/_totalConstitunent;
